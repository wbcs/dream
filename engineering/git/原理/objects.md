...
一个树对象（和 unix 中的 inodes 类似）包含了一条或多条树对象记录（tree entry），每条记录含有一个指向数据对象或者子树对象的 `SHA-1` 指针，以及相应的模式、类型、文件名信息.
...
object|存储信息|区别
-|-|-
commit|父 commit,tree,提交时间,注释,提交者|指针指向 tree
tag|commit,时间,注释,创建者|指针指向 commit

它像是一个永不移动的分支引用(都位于 refs 下)——永远指向同一个提交对象，只不过给这个提交对象加上一个更友好的名字罢了。
...
当然，为了节省磁盘空间，git 还将最终要保存的内容做了 zlib 压缩，然后取得 hash 的前 2 个字母作为目录名称，其余作为文件名称，最后存储至 `.git/objects` 下。
...
- `git commit-tree $HASH_OF_TREE [-p]`: 提交当前的 tree-object ，-p 是指定其父 commit-object 的 hash
