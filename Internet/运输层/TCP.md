...
## TCP的三次握手🤝
`TCP`建立连接的过程叫做握手，握手需要在`client`和`server`之间交换3个`TCP`报文段。
...
# TCP的可靠连接原理
我们都知道`TCP`是提供可靠传输的运输层协议，但是在其之下的IP协议却是不保证可靠传输的协议。那么`TCP`是如何实现在不可靠的IP层上实现可靠传输的呢？
...
### client
`client`的发送窗口在没有收到`server`的确认的情况下，可以把发送窗口内的数据都发送出去，没法送1byte，p2就向前滑动1个单位，直到p2和p3重合则停止发送（也就是可用窗口大小为0）。等到收到`server`的确认后，就可以将此数据删除掉。然后将发送窗口向前滑动（p1、p3向前移动，p2不变），这个时候可用窗口增大，client就又能发送数据了。
