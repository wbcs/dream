...
HTTP1.1引入100(Continue)新状态码。客户端先发送一个只带头部的请求，如果权限不足被Server拒绝(401 Unauthorized)，反之Server响应100告诉客户端可以发送带实体的完整请求，此时客户端再发送完整的实体就可以了。客户端在请求头中需要添加`Expect: 100-continue`。
